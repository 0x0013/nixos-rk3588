...
For NanoPi R6S, follow the [R6S prerequisites and validation notes](./README.md#nanopi-r6s-validation-status)
and build `nix build .#rawEfiImage-nanopi-r6s`. Use R6S-specific firmware in
Device Tree / Vendor mode and preserve its storage layout when writing OS media.
The image uses systemd-boot's per-generation DTB. The SPI flashing and fixed-DTB
override steps below are not an R6S installation procedure.
...
nix build github:gnull/nixos-rk3588#rawEfiImage-opi5plus --show-trace -L --verbose

# for orange pi 5
nix build github:gnull/nixos-rk3588#rawEfiImage-opi5 --show-trace -L --verbose

# for rock 5a
nix build github:gnull/nixos-rk3588#rawEfiImage-rock5a --show-trace -L --verbose
```
...
4. Combine the generated configuration with the [demo configuration](./examples/demo) in this repo, finish
...
> gpu drivers, etc. The [demo configuration](./examples/demo) shows how to reference the necessary
